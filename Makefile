...
GFXBUILD	:=	$(ROMFS)/gfx
APP_AUTHOR	:=	NPI-D7
APP_DESCRIPTION :=      Test Stupid Stuff on 3ds
ICON		:=	app/icon.png
...
$(OUTPUT).cia	:	$(OUTPUT).elf $(OUTPUT).smdh
	...
	$(MAKEROM) -f cia -target t -exefslogo -o "../TestProject.cia" -elf "../TestProject.elf" -rsf "../app/build-cia.rsf" -banner "../app/banner.bin" -icon "../app/icon.bin" -logo "../app/logo.lz11" -DAPP_ROMFS="$(TOPDIR)/$(ROMFS)" -major $(VERSION_MAJOR) -minor $(VERSION_MINOR) -micro $(VERSION_MICRO) -DAPP_VERSION_MAJOR="$(VERSION_MAJOR)"
#---------------------------------------------------------------------------------
